package Control::Plugin::Pagination;
...
sub  register{
  ...
  $app->helper( pagination => sub{
      ...
      my $html = "<div class=\"pages-box\">";
      if( $actual == $start ){
        $html .= "<a href=\"#\" class=\"square-button\">&laquo;</a>";
      } else {
        $html .= "<a href=\"" . $self->url_with->query( [$param => $actual - 1] ) . $query . "\" class=\"square-button\">&laquo;</a>";
      }
      my $last_num = -1;
      foreach my $number( @ret ){
        ...
        if( $number eq ".." && $last_num < $actual ){
          ...
          $html .= "<a href=\"" . $self->url_with->query( [$param => $start == 0 ? $offset + 1 : $offset] ) . $query ."\" class=\"square-button\">&hellip;</a>";
        }
          ...
          # select!!!!
          $html .= "<a href=\"\" class=\"square-button active\">". $show_number ."</a>";
        } else {
          $html .= "<a href=\"" . $self->url_with->query( [$param => $number] ) . $query ."\" class=\"square-button\">$show_number</a>";
        }
         $last_num = $number;
      }
      if( $actual == $count ){
